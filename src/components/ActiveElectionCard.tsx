...
import { MdArrowForward, MdVolumeUp, MdVerified } from "react-icons/md";
...
export function ActiveElectionCard({ id, title, description, endTime, onVote }: ActiveElectionCardProps) {
  ...
  const { data: hasVoted, isLoading: isLoadingVoteStatus } = useVoteStatus(id);
      ...
      <div className="relative z-10 flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1">
          ...
          {isLoadingVoteStatus ? (
            <button
              disabled
              className="bg-[#234848] text-[#92c9c9] font-bold px-6 py-3 flex items-center gap-2 transition-all opacity-50 cursor-not-allowed"
            >
              <span>Loading...</span>
            </button>
          ) : hasVoted ? (
            <button
              onClick={() => navigate(`/vote-verification?electionId=${id}`)}
        ...
        <div className="flex flex-col">
          ...
          <div className={`grid ${showDays ? 'grid-cols-2' : 'grid-cols-2'} gap-3 mb-4`}>
            {/* Days - only show if more than 24 hours remaining */}
            {showDays && (
            ...
            {/* Urgent indicator - only show when less than 24 hours remaining */}
            {!showDays && (
